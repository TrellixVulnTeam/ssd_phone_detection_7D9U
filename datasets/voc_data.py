...
def ssd_random_sample_crop( image,img_size, center_bbox_labels,sample_options=(None,(0.8, None))):
    ...
    while True:
        ...
        for _ in range(50):
            ...
            overlap = 0
            if intersect(boxes, rect)>0:
                overlap_boxes = np.zeros_like(boxes)
                overlap_boxes[:, 2:] =  np.minimum(boxes[:, 2:], rect[2:])
                overlap_boxes[:, :2] = np.maximum(boxes[:, :2], rect[:2])
                overlap = jaccard_numpy(boxes,overlap_boxes[0])

                


            # is min and max overlap constraint satisfied? if not try again
            min_overlap = 0 if overlap==0 else overlap.min()

            if overlap==0 or overlap.min() < min_iou :
                continue
...
class VOCFormatDetectionDataset(VisionDataset):
    def __init__(
            ...
            voc_label_map={'phone':0,'cell phone':0} #'phone':0,'cell phone':0,
            ):
