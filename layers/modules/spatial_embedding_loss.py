...
class SpatialEmbeddingLoss(nn.Module):
    ...
    def __init__(self, to_center=True, n_sigma=1, size=(2048,1024),foreground_weight=1):
        super(SpatialEmbeddingLoss,self).__init__()
        ...
        # coordinate map
        xm = torch.linspace(0, 2, size[0]).view(
            1, 1, -1).expand(1, size[1], size[0])
        ym = torch.linspace(0, 1, size[1]).view(
            1, -1, 1).expand(1, size[1], size[0])
        xym = torch.cat((xm, ym), 0)

        self.register_buffer("xym", xym)

    def forward(self, prediction, inst_labels, seg_labels, w_inst=1, w_var=10, w_seed=1, iou=False, iou_meter=None):
        ...
        for b in range(0, batch_size):
            ...
            instance = inst_labels[b].unsqueeze(0)  # 1 x h x w
            label = seg_labels[b].unsqueeze(0)  # 1 x h x w
